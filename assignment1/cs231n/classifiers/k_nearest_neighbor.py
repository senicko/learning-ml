...
class KNearestNeighbor(object):
    ...
    def compute_distances_no_loops(self, X):
        ...
        dists = np.sqrt(
            ...
            - 2 * (X @ self.X_train.T)
            + np.sum(np.square(self.X_train), axis=1)[np.newaxis, :]
        )
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            # Select first k indexes that would sort the i'th row of dists matrix
            # and then use these indexes to index self.y_train to retrieve labels
